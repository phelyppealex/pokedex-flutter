    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Pokemon` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `nome` TEXT NOT NULL, `especie` TEXT NOT NULL, `altura` INTEGER NOT NULL, `peso` INTEGER NOT NULL, `habilidade` TEXT NOT NULL, `tipo` TEXT NOT NULL, `imagem` TEXT NOT NULL)');
...
class _$PokemonDao extends PokemonDao {
  ...
  @override
  Future<List<Pokemon>> listarTodos() async {
  ...
  @override
  Future<List<Pokemon>> listarPorId(int id) async {
    return _queryAdapter.queryList('SELECT * FROM Pokemon WHERE id = ?1',
        mapper: (Map<String, Object?> row) => Pokemon(
            id: row['id'] as int?,
            nome: row['nome'] as String,
            especie: row['especie'] as String,
  ...
  @override
  Future<void> deletarPokemon(Pokemon p) async {
    await _pokemonDeletionAdapter.delete(p);
  }
}
